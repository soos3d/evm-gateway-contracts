 ...
 * @dev This contract is for internal local blockchain testing purposes only.
 */
contract Create2Factory is Ownable {
    /**
     * @notice Initializes the contract with an owner who can perform deployments
     ...
     */
    function deploy(uint256 amount, bytes32 salt, bytes calldata bytecode)
        external
        payable
        onlyOwner
     ...
     * @return addr The deployed address
     */
    function deployAndMultiCall(uint256 amount, bytes32 salt, bytes calldata bytecode, bytes[] calldata data)
        external
        payable
        onlyOwner
        returns (address addr)
